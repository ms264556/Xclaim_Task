#ifndef __LINUX_AR9130_OTG_H
#define __LINUX_AR9130_OTG_H

#include "../gadget/ar9130_defs.h"

#define USB_lock()          spin_lock_irqsave(&ar9130->arlock, arflags)
#define USB_unlock()        spin_unlock_irqrestore(&ar9130->arlock, arflags)
...
#define START_TIMER(x)                                                      \
   {                                                                        \
      u32  temp;                                                            \
      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                    \
      writel((temp | AR9130_OTGSC_1MSIE), &(x)->otgsc);                     \
      (ar9130->enabled_ints |= AR9130_OTGSC_1MSIE);                         \
   }
 ...
 */
#define ID_CHG(mask)          (mask & AR9130_OTGSC_IDIS)
#define SESS_VLD_CHG(mask)    (mask & (AR9130_OTGSC_ASVIS | AR9130_OTGSC_BSVIS))
#define A_VBUS_CHG(mask)      (mask & AR9130_OTGSC_AVVIS)
...
#define SESS_VLD_FALSE(x)     \
   (!(readl(&(x)->otgsc) & (AR9130_OTGSC_BSV | AR9130_OTGSC_ASV)))
   
#define SESS_VLD(x)           \
   (readl(&(x)->otgsc) & (AR9130_OTGSC_BSV | AR9130_OTGSC_ASV))
   
#define B_SESS_END(x)         \
   (readl(&(x)->otgsc) & AR9130_OTGSC_BSE)

#define ID(x)                 \
   (readl(&(x)->otgsc) & AR9130_OTGSC_ID)
   
#define ID_FALSE(x)           \
   (!(readl(&(x)->otgsc) & AR9130_OTGSC_ID))
   
#define A_VBUS_VLD(x)         \
   (readl(&(x)->otgsc) & AR9130_OTGSC_AVV)
...
#define SET_STATE(stat, ordinal)		 ar9130->otg.state = (stat)
...
#define VBUS_DSCHG_ON(x)                                                    \
   {                                                                        \
      u32  temp;                                                            \
      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                    \
      writel((temp | AR9130_OTGSC_VD), &(x)->otgsc);                        \
   }
...
#define VBUS_CHG_ON(x)                                                      \
   {                                                                        \
      u32  temp;                                                            \
      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                    \
      writel((temp | AR9130_OTGSC_VC), &(x)->otgsc);                        \
   }
...
#define DP_HIGH_ON(x)                                                       \
   {                                                                        \
      u32 temp;                                                             \
      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                    \
      writel((temp | AR9130_OTGSC_HADP), &(x)->otgsc);                      \
   }
...
#define DP_HIGH_ON(x)         \
   { \
      uint_32  temp; \
      temp = ((x)->REGISTERS.OPERATIONAL_HOST_REGISTERS.OTGSC & ~AR9130_OTGSC_INTERRUPT_STATUS_BITS_MASK); \
      ((x)->REGISTERS.OPERATIONAL_HOST_REGISTERS.OTGSC = (temp | AR9130_OTGSC_DP)); \
   }
...
#define AUTO_RESET_ON(x)                                                    \
   {                                                                        \
      u32 temp;                                                             \
      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                    \
      writel((temp | AR9130_OTGSC_HAAR), &(x)->otgsc);                      \
   }
...
#define IS_AUTO_RESET_ON(x,y)                                               \
   {                                                                        \
      u32 temp;                                                             \
      temp = readl(&(x)->otgsc);                                            \
      y = ((temp & AR9130_OTGSC_HAAR)? TRUE : FALSE);                       \
   }
...
#define AUTO_HNP_ON(x)                                                      \
   {                                                                        \
      u32 temp;                                                             \
      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                    \
      writel((temp | AR9130_OTGSC_HABA), &(x)->otgsc);                      \
   }
...
#define IS_AUTO_HNP_ON(x,y)                                                 \
   {                                                                        \
      u32 temp;                                                             \
      temp = readl(&(x)->otgsc);                                            \
      y = ((temp & AR9130_OTGSC_HABA)? TRUE : FALSE);                       \
   }

#define  AR9130_MODE_DEV                 (0x00000002)

#define IS_DEVICE_MODE_ON(x,y)                                              \
   {                                                                        \
      u32 temp;                                                             \
      temp = (readl(&(x)->usbmode) & 0x3);                                  \
      y = ((temp == AR9130_MODE_DEV)? TRUE : FALSE);                        \
   }
 ...
 */
#ifdef PATCH_3
    /*
     * WEB20040409 below changed to reflect  AR9130_OTGSC_B_HOST_EN is no 
     * longer used. 
	      temp = ((x)->REGISTERS.OPERATIONAL_HOST_REGISTERS.OTGSC & ~AR9130_OTGSC_INTERRUPT_STATUS_BITS_MASK); \
	      ((x)->REGISTERS.OPERATIONAL_HOST_REGISTERS.OTGSC = (temp | AR9130_OTGSC_B_HOST_EN));
          */
	/* to this */ 
	...
	#define PULL_UP_PULL_DOWN_HOST_CTRL(x)                                  \
	   {                                                                    \
	      u32  temp;                                                        \
	      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                \
	      writel((temp | AR9130_OTGSC_BHEN), &(x)->otgsc);                  \
	   }
#endif      
...
#define PULL_UP_PULL_DOWN_DEVICE_CTRL(x)                                    \
   {                                                                        \
      u32  temp;                                                            \
      temp = (readl(&(x)->otgsc) & ~AR9130_OTGSC_SMASK);                    \
      writel((temp | AR9130_OTGSC_OT), &(x)->otgsc);                        \
   }

#define PULL_UP_PULL_DOWN_IDLE(x)

#define OTG_HOST_INIT(x, y)                                                 \
   ar9130_host_resume((x));                                                 \
