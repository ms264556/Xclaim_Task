 ...
 *
 *  $Id: //depot/sw/releases/Aquila_9.2.0/linux/kernels/mips-linux-2.6.15/drivers/serial/hornet_serial.c#1 $
 *
...
#include "8250.h"
#if defined(CONFIG_V54_BSP)
#include <linux/proc_fs.h>
#include "ar7100.h"
#include "hornet.h"
#define CONFIG_MACH_HORNET
//#define CONFIG_SERIAL_HORNET_CONSOLE
#define CONFIG_SERIAL_HORNET_NR_UARTS   1
#define UARTCLOCK_9600                  0x001a054f
#define UARTCLOCK_19200                 0x000D0581
#define UARTCLOCK_115200                0x00020714
#define UARTCLOCK_460800                0x0000096F
int serial8250_register_port_hornet(struct uart_port *port);
void serial8250_unregister_port_hornet(int line);
static int txfifo_cnt         = 0;
static int rx_full_cnt        = 0;
static int rx_breakon_cnt     = 0;
static int rx_overflow_cnt    = 0;
static int rx_frameerror_cnt  = 0;
#define RX_ERR_INT 
#else
#include "ar7240.h"
...
static void autoconfig(struct uart_8250_port *up, unsigned int probeflags)
{
	...
	DEBUG_AUTOCONF("ttyHS%d: autoconf (0x%04x, 0x%p): ",
...
//	save_flags(flags); cli();
	...
	if (up->capabilities != uart_config[up->port.type].flags) {
		printk(KERN_WARNING
		       "ttyHS%d: detected caps %08x should be %08x\n",
...
#if defined(CONFIG_V54_BSP)
    rdata |= UARTCS_UARTFLOWCONTROLMODE_SET(2); // 0: no 2: turn on 3: inverted 
    rdata |= UARTCS_UARTINTERFACEMODE_SET(2);   // 1: DTE, 2: DCE
#endif
    serial_out(up, UARTCS_ADDRESS, rdata);
            
    //rdata = serial_in(up, UARTINTEN_ADDRESS);
    rdata = (UARTINTEN_UARTRXVALIDINTEN_SET(1));

#ifdef RX_ERR_INT
    rdata |= UARTINTEN_UARTRXFULLINTEN_SET(1);    
    rdata |= UARTINTEN_UARTRXBREAKONINTEN_SET(1);
    rdata |= UARTINTEN_UARTRXOFLOWERRINTEN_SET(1);
    rdata |= UARTINTEN_UARTRXFRAMINGERRINTEN_SET(1);
#endif

    serial_out(up, UARTINTEN_ADDRESS, rdata); 
#if defined(CONFIG_V54_BSP)
    serial_out(up, UARTCLOCK_ADDRESS, UARTCLOCK_115200);
#endif
}
...
static inline void __stop_tx(struct uart_8250_port *p)
{
	...
	if (p->ier & UART_IER_THRI) {
        ...
        rdata = rdata & ~(UARTINTEN_UARTRXVALIDINTEN_SET(1));
...
#if defined(V54_BSP)
    /* transmit_chars will call wait_for_xmitr to check uart1_tx_csr */
    transmit_chars(up);
#else
...
#define BIT6 (0x1<<6)
    ...
    //ar7240_reg_rmw_clear(AR7240_MISC_INT_MASK, BIT6);
...
#else
    ar7240_reg_rmw_clear(AR7100_MISC_INT_STATUS, BIT6);
#endif
...
#ifdef RX_ERR_INT
    uart_tcs_s |= UARTINTEN_UARTRXFULLINTEN_SET(1);    
    uart_tcs_s |= UARTINTEN_UARTRXBREAKONINTEN_SET(1);
    uart_tcs_s |= UARTINTEN_UARTRXOFLOWERRINTEN_SET(1);
    uart_tcs_s |= UARTINTEN_UARTRXFRAMINGERRINTEN_SET(1);
#endif
    ...
    /* 8. set misc int mask */
    //ar7240_reg_wr(AR7240_MISC_INT_MASK, BIT6);
}
...
serial8250_handle_port(struct uart_8250_port *up)
{
    ...
    DPRINTK("%s: int_status(%x) en_status(%x) status(%x)\n",
        __func__, int_status, en_status, status);
#ifdef RX_ERR_INT
	if( (int_status&en_status) & UARTINT_UARTRXFULLINT_MASK) {
            rx_full_cnt++;
        }
	if( (int_status&en_status) & UARTINT_UARTRXBREAKONINT_MASK) {
            rx_breakon_cnt++;
        }
	if( (int_status&en_status) & UARTINT_UARTRXOFLOWERRINT_MASK) {
            rx_overflow_cnt++;
        }
	if( (int_status&en_status) & UARTINT_UARTRXFRAMINGERRINT_MASK) {
            rx_frameerror_cnt++;
        }
#endif
 ...
 */
static irqreturn_t serial8250_interrupt_hornet(int irq, void *dev_id)
{
#ifdef CONFIG_MACH_HORNET
...
static int serial_link_irq_chain(struct uart_8250_port *up)
{
	...
	if (i->head) {
		...
		ret = request_irq(up->port.irq, serial8250_interrupt_hornet,
				  irq_flags, "serial_hornet", i);
		if (ret < 0)
 ...
 */
void serial8250_suspend_port_hornet(int line)
{
 ...
 */
void serial8250_resume_port_hornet(int line)
{
...
static int __devinit serial8250_probe(struct platform_device *dev)
{
	...
	for (i = 0; p && p->flags != 0; p++, i++) {
		...
		ret = serial8250_register_port_hornet(&port);
		if (ret < 0) {
...
static int __devexit serial8250_remove(struct platform_device *dev)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		if (up->port.dev == &dev->dev)
			serial8250_unregister_port_hornet(i);
	}
	return 0;
}
...
EXPORT_SYMBOL(serial8250_register_port_hornet);
...
EXPORT_SYMBOL(serial8250_unregister_port_hornet);

#if defined(V54_BSP)
static int proc_hs_uart_stats (char* page, char ** start,
            off_t off, int count, int *eof, void *data) 
{
    int len=0;
#ifdef RX_ERR_INT
    len = sprintf(page, "tx full     = %d\nrx full     = %d\nrx breakon  = %d\nrx overflow = %d\nrx framing  = %d\n",
                        txfifo_cnt, rx_full_cnt, rx_breakon_cnt, rx_overflow_cnt, rx_frameerror_cnt);
#else
    len = sprintf(page, "tx full     = %d\n", txfifo_cnt);
#endif
    return len;
...
static int __init serial8250_init_hornet(void)
{
	int ret, i;
#if defined(V54_BSP)
    static struct proc_dir_entry *stats, *hs_uart;
#endif

	printk(KERN_INFO "Serial Hornet: 8250/16550 driver $Revision: #1 $ "
...
#if defined(V54_BSP)
    hs_uart = proc_mkdir("hs_uart", NULL);

    /* create proc: proc/hs_uart/stats */
    stats = create_proc_entry("stats", 0444, hs_uart);

    /* register read function */
//    stats->owner = THIS_MODULE;
    stats->read_proc = proc_hs_uart_stats;
    stats->data = NULL;
#endif

	serial8250_isa_devs = platform_device_register_simple("serial8250_hornet",
...
static void __exit serial8250_exit_hornet(void)
{
...
module_init(serial8250_init_hornet);
module_exit(serial8250_exit_hornet);

EXPORT_SYMBOL(serial8250_suspend_port_hornet);
EXPORT_SYMBOL(serial8250_resume_port_hornet);
