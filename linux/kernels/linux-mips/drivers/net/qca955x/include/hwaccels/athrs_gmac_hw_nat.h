...
#ifndef _ATHR_GMAC_HW_NAT_H
#define _ATHR_GMAC_HW_NAT_H
...
#define EG_INFO_INS_DW1_REG           0xc         /* offset is defined  only for scoprion and in WASP this reg will not be accesed  */
//#define EG_INFO_STAT_DW0_REG          0xc         /* this macro not used */ 
#define EG_INFO_WAN_PORT(x)           (x << 0)    /* INFO data elements */
#define EG_INFO_WAN_IP_IDX(x)         (x << 16)   /* WAN IP ADDR select */
#define EG_INFO_CONN_STAT(x)          (x << 18) /* it should be 18 for both wasp and scorpion */ 

#define EG_INFO_SW_INFO(x)            (x >> 11) 

#ifdef CONFIG_MACH_QCA955x || CONFIG_MACH_QCA955x_MODULE
#define EG_KEY_DW0_REG                0x18
#define EG_KEY_DW1_REG                0x1c
#define EG_KEY_DW2_REG                0x20
#define EG_KEY_DW3_REG                0x24
#else
...
#ifdef CONFIG_MACH_QCA955x || CONFIG_MACH_QCA955x_MODULE
#define EG_KEY_DW1_WAN_ADDR(x)        ((x & 0x3fff) << 18)
#define EG_KEY_DW2_WAN_ADDR(x)        ((x >> 14) & 0x3ffff)
#define EG_KEY_DW2_WAN_PORT(x)        ((x & 0x3fff) << 18)
#define EG_KEY_DW3_WAN_PORT(x)        ((x >> 14) & 0x3)
#endif
...
#define IG_INFO_DW3_CONN_STAT(x)      ((x & 0x7) << 0)
#define IG_INFO_DW3_SW_INFO(x)        ((x & 0xFFF) << 3)

#define IG_KEY_DW0_REG                0x28
#define IG_KEY_DW0_WAN_PORT(x)        (x << 0)    /* WAN port Number */
#define IG_KEY_DW0_PROT(x)            ((x & 3) << 16)   /* Protocol key */
#define IG_KEY_DW0_WAN_IP_IDX(x)      (x << 18)   /* WAN IP ADDR select */

#ifdef CONFIG_MACH_QCA955x || CONFIG_MACH_QCA955x_MODULE
#define IG_KEY_DW1_REG                0x2c
#define IG_KEY_DW2_REG                0x30
#define IG_KEY_DW0_LAN_IP_ADDR(x)     ((x & 0xFFF) << 20)
#define IG_KEY_DW1_LAN_IP_ADDR(x)     ((x >> 12) & 0xFFFFF)
#define IG_KEY_DW1_LAN_PORT(x)        ((x & 0xFFF) << 20)
#define IG_KEY_DW2_LAN_PORT(x)        ((x >> 12) & 0xF)
#endif
...
typedef struct {
   ...
   uint32_t wan_ip_addr;                   /* Used for lookup */
   uint16_t wan_port;
   uint32_t sw_info;
...
typedef struct {
   ...
   uint32_t wan_src_addr;                   /* Used for lookup */
...
#ifdef CONFIG_MACH_QCA955x || CONFIG_MACH_QCA955x_MODULE
#define CFG_EG_INIT 0x3000
#define CFG_IG_INIT 0x180000
#else
#define CFG_EG_INIT 0xc
#define CFG_IG_INIT 0xc
#endif
