...
#define is_8021() 0
#define is_vir_phy() 0
#define is_ar8033_phy() 0
#define is_ar7241_abv() (is_ar7241() || is_ar7242() || is_ar934x() || is_qca953x())
...
athr_gmac_phy_attach(void *arg,int unit)
{
   ...
   if ((unit == 1) && is_qca955x())
#else
   if (unit == 0 && (is_ar934x() || is_qca955x()))
#endif // AP136_BOOTROM_HOST
#else
   if ((unit == 0  || unit == 1) && (is_qca955x() || is_ar934x())) 
#endif
   {
...
#else
   if((is_ar934x() || is_qca953x() || is_qca955x()) && (unit == 0)){
#endif
...
#ifdef CONFIG_ATHR_SUPPORT_DUAL_PHY
#if defined (AP136_BOOTROM_HOST) || defined (GMAC0_MII)
   if (is_qca955x() && (unit == 0))
#else
   if ((is_ar934x() || is_qca953x() || is_qca955x()) && (unit == 1 ))
#endif // AP136_BOOTROM_HOST
#else
   if ((is_ar934x() || (is_qca953x() && (unit ==1) ) || is_qca955x()))
#endif
    {
