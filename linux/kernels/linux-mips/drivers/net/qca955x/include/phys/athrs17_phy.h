...
#define S17_QM_CTRL_FLOW_DROP_DIS       0X4E
#define S17_QM_CTRL_FLOW_DROP_EN        0X18
#endif
...
#define S17_MAC0_RGMII_RXCLK_SEL	21
...
#if defined(V54_BSP)
// QCA8337
#define S17_HOL_CTRL0_LAN		0x22008642 /* egress priority 8, eg_portq = 0x22 */
#define S17_HOL_CTRL0_WAN		0x22864200 /* egress priority 8, eg_portq = 0x22 */
#define S17_HOL_CTRL1_DEFAULT		0x1c2	   /* enable HOL control */
#else
...
/* enable 802.3az */
#if !defined(CONFIG_MACH_QCA955x) && !defined(CONFIG_MACH_QCA955x_MODULE)
...
#define PORT0_BITMAP    (1<<0)
#define PORT1_BITMAP    (1<<1)
#define PORT2_BITMAP    (1<<2)
#define PORT3_BITMAP    (1<<3)
#define PORT4_BITMAP    (1<<4)
#define PORT5_BITMAP    (1<<5)
#define PORT6_BITMAP    (1<<6)
#define PORT_ALL_BITMAP (0x7f)
...
typedef enum
{
    ...
    GMII_SINGLE_PHY,
    DONT_CARE,
} athr_switch_intf_type_t;

typedef struct
{
    athr_switch_intf_type_t mac0_type; /* type for mac 0 */
    athr_switch_intf_type_t mac6_type; /* type for mac 6 */
    athr_switch_intf_type_t mac5_type; /* not use */
    int wan_port;  /* WAN port */
    int vlan_id;   /* vlan_id for LAN, vlan for WAN = vlan for LAN +1 */
    int wan2_port; /* for dual wan, not use now */
    int use_vlan_table; /* use tag vlan, not port base vlan */
} athr_switch_intf_t;
void athrs17_reg_config(int flag, BOOL is_lan);
int athrs17_reg_init(void *arg);
#if HYBRID_LINK_CHANGE_EVENT
int athrs17_phy_is_up(int ethUnit, int *rc);
#else
