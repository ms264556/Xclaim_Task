...
extern void aos_header_arp_in( uint32_t mac,struct sk_buff *skb);

void athr_ssdk_process_arp_header(athr_gmac_t *mac, struct sk_buff *skb)
{
        aos_header_arp_in(mac->mac_unit,skb);
}
...
#else/* For HB, Station addr needs to be 6:5:4:3 and then 2:1 */
 #define addr_to_words(addr, w1, w2)  {                                 \
...
#ifdef CONFIG_ATH_JUMBO
#define athr_jumbo_fr() len = ((ds->pkt_size & 0xffff) |\
			((ds->res3 & 0x03) << 12));
#define athr_alloc_skb()  skb = dev_alloc_skb((ATHR_GMAC_RX_BUF_SIZE + 2 + frame_sz) + L1_CACHE_BYTES - 1)
#else
#define athr_jumbo_fr() len = ds->pkt_size;
#define athr_alloc_skb()  skb = dev_alloc_skb((ATHR_GMAC_RX_BUF_SIZE + 2) + L1_CACHE_BYTES - 1) 
#endif
