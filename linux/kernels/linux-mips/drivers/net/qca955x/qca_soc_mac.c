...
#define QCA955x_MAX_MTU     2290
static int athr_gmac_change_mtu(struct net_device *dev, int new_mtu)
{
    athr_gmac_t *mac = *(athr_gmac_t **)ATHR_MAC_PRIV(dev);
    athr_phy_ops_t *phy = mac->phy;

    if (new_mtu < 68 || new_mtu > QCA955x_MAX_MTU)
        return -EINVAL;
    ...
    frame_sz = new_mtu + ETH_HLEN + 4 + ATHR_HEADER_LEN;
    ...
    if(!is_f1e())
    {
        ...
        if (is_s16() && is_s17())
            phy->write_mac_reg(0x78, (phy->read_mac_reg(0x78)& ~0x3FFF) | frame_sz);
        else if(is_s16())
            phy->write_mac_reg(0x30, (phy->read_mac_reg(0x30)& ~0x3FFF) | frame_sz);
        else if(is_s26())
            phy->write_mac_reg(0x30, (phy->read_mac_reg(0x30)& ~0x7FF)  | frame_sz);
        else
            phy->write_mac_reg(0x30, (phy->read_mac_reg(0x30)& ~0x3FFF) | frame_sz);
    }
    printk("\nframe_sz=%d\n", frame_sz);
...
athr_gmac_init(void)
{
...
#if defined(V54_BSP)
    printk("%s: NMACS(%d) totalDev(%d)\n", __func__, ATHR_GMAC_NMACS, totalDev);
#endif
    /* 
    ...
    */
    ...
    for(i = 0; i < ((totalDev < ATHR_GMAC_NMACS) ? totalDev : ATHR_GMAC_NMACS); i++)
    {
        ...
        // allocate network device and private data space - pointer to mac
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31))
        athr_gmac_net_ops.ndo_change_mtu = athr_gmac_change_mtu;
#endif
