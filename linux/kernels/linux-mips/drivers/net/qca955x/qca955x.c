...
int qca955x_gmac_poll_link(athr_gmac_t *mac)
{
    ...
    switch (mac->mii_intf) {
        case ATHR_RGMII:
#if !defined(V54_BSP)
		athr_get_rgmii_link_st(); 
#else
#if defined(CONFIG_ATHRF1_PHY)
            // qca8035
            phyUnit = 0;
#else
            // qca8337 wan port
...
#endif
            athr_get_rgmii_link_st(); 
            //printk("%s: phy %d up %d speed %d fdx %d\n", __func__, phyUnit, phy_up, speed, fdx);
#if !defined(CONFIG_ATHRF1_PHY)
            phy_up = (phy_up & 0x10) ? 1 : 0;
#endif
            current_conn[mac->mac_unit].up = phy_up;
...
//		fdx =1; 		
//#else 
  //          athr_gmac_get_link_st(mac, &phy_up, &fdx, &speed, phyUnit);
// #endif 
            break;
#if defined(CONFIG_ATHRS_GMAC_SGMII)
        case ATHR_SGMII:
...
#if defined(V54_BSP)
        if (mac->mii_phy == ATHR_F1E_PHY) {
            ath_reg_wr(ETH_XMII_ADDRESS, ETH_XMII_TX_INVERT_SET(1) |
                ETH_XMII_RX_DELAY_SET(2)  |
                ETH_XMII_TX_DELAY_SET(1)  |
                ETH_XMII_GIGE_SET(1)  |
                ETH_XMII_PHASE1_COUNT_SET(0)  |
                ETH_XMII_PHASE0_COUNT_SET(0));
            ath_reg_wr(ATHR_GMAC_ETH_CFG,
                        ETH_CFG_ETH_RXDV_DELAY_SET(3)  |                
    ...
    case ATHR_PHY_SPEED_100T:
        ...
        if (mac->mii_phy == ATHR_F1E_PHY) {
            ath_reg_wr(ETH_XMII_ADDRESS, ETH_XMII_TX_INVERT_SET(1) |
                ...
                ETH_XMII_PHASE0_COUNT_SET(1));
            ath_reg_wr(ATHR_GMAC_ETH_CFG, ETH_CFG_RGMII_GE0_SET(1));
        }
#endif
        break;
