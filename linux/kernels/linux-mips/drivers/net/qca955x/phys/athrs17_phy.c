...
static BOOL _athrs17_mac0_intf(athr_switch_intf_type_t type)
{
    ...
    switch (type)
    {
        ...
        case SGMII:
            ...
            // QCA8337: Suggested in QCA8337 FAQ but caused rx packets dropped in 2DVPT.
            // athrs17_reg_write(S17_P5PAD_MODE_REG , 0x01000000); /* RGMII RXCLK is delayed */
#else
...
#if defined(V54_BSP)
    athrs17_reg_write(S17_LED_CTRL0_REG, 0xcd35cd35);
    athrs17_reg_write(S17_LED_CTRL1_REG, 0xcb35cb35);
#endif
    athrs17_reg_write(S17_LED_CTRL2_REG, 0xffb7ffb7);

#if defined(V54_BSP)
    // QCA8337
    athrs17_reg_write(S17_QM_CTRL_REG, S17_QM_CTRL_FLOW_DROP_DIS);
#endif
...
#else
            if (phyNum == phyUnit) {
                return ((phyHwStatus & S17_STATUS_FULL_DEPLEX) ? 1 : 0);
            }
#endif
        }
    }
