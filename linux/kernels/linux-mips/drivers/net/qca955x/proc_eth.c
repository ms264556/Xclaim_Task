        ...
        //if (rks_is_gd26_board() || rks_is_spaniel_board() || rks_is_gd36_board()) {
            if (!i) {
                ...
                len += sprintf(bp+len, "%d %s %d %s %s\n", ethname_remap[i], ethname_array[ethname_remap[1]],
                        (current_conn[1].up),
                        (current_conn[1].up) ? spd_str[current_conn[1].speed] : "",
                        (current_conn[1].up) ? (current_conn[1].fdx ? "full" : "half") : "");
            }
        //} 
    }     
    return len;
    ...
    for( i=0; i < num_macs; i++ ) {
        ...
        //if (rks_is_gd26_board() || rks_is_spaniel_board() || rks_is_gd36_board()) {
            if (!i) {
                ...
                len += sprintf(bp+len, "%d %d\n", ethname_remap[i], current_conn[1].up);
            }
        //} 
    }    
    return len;
...
#if defined(CONFIG_ATHRS17_PHY)
LOCAL int
proc_write_rdsw(struct file *file, const char *buffer,
                unsigned long buffer_length, void *data)
{
    unsigned short len;
    int i, retval;
...
LOCAL int
proc_read_rdsw(char* page, char ** start,
            off_t off, int count, int *eof, void *data)
{
    int len=0;
...
LOCAL int
proc_write_wrsw(struct file *file, const char *buffer,
                unsigned long buffer_length, void *data)
{
    unsigned int len, reg, val;
    int i, retval;
    char *pEnd, *pBegin;
...
LOCAL int
proc_read_wrsw(char* page, char ** start,
            off_t off, int count, int *eof, void *data)
{
    return 0;
}
            ...
            off_t off, int count, int *eof, void *data)
{
    ...
    for(offset=0x80; offset<=0x13c; offset+=0x4) {
        len += sprintf(bp+len, "%3x:", offset);        
        for( i=0; i < num_macs; i++ ) {
            mac = athr_gmacs[i];
            val = athr_gmac_reg_rd(mac, offset);
    ...
    for(offset=0x180; offset<=0x1ac; offset+=0x4) {
        len += sprintf(bp+len, "%3x:", offset);        
        for( i=0; i < num_macs; i++ ) {
            mac = athr_gmacs[i];
            val = athr_gmac_reg_rd(mac, offset);
    ...
    for(offset=0x1c0; offset<=0x1d8; offset+=0x4) {
        len += sprintf(bp+len, "%3x:", offset);        
        for( i=0; i < num_macs; i++ ) {
            mac = athr_gmacs[i];
            val = athr_gmac_reg_rd(mac, offset);
...
#if defined(CONFIG_ATHRS17_PHY)
#define RDSW_FILE    "rdsw"
LOCAL struct proc_dir_entry *rdsw_file;

#define WRSW_FILE    "wrsw"
LOCAL struct proc_dir_entry *wrsw_file;
#endif
...
#if defined(CONFIG_ATHRS17_PHY)
    if (CREATE_RDWR_PROC(proc_enet, &rdsw_file, RDSW_FILE,
		        proc_read_rdsw, proc_write_rdsw) != 0 ) {
        printk("%s: create_proc_entry(%s/%s) failed\n", __FUNCTION__,
                PROC_NET_ETH, RDSW_FILE);
    }
    if (CREATE_RDWR_PROC(proc_enet, &wrsw_file, WRSW_FILE,
		        proc_read_wrsw, proc_write_wrsw) != 0 ) {
        printk("%s: create_proc_entry(%s/%s) failed\n", __FUNCTION__,
                PROC_NET_ETH, WRSW_FILE);
    }
#endif
    return;
}
