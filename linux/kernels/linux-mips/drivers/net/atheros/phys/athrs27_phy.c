...
#if defined(V54_BSP)
        athrs27_reg_write(S27_PORT_CONTROL_REGISTER0, \
            ( S27_PORT_MODE_FWD | S27_HEADER_EN));
#else
        athrs27_reg_write(S27_PORT_CONTROL_REGISTER0, \
            ( S27_LEARN_EN | S27_PORT_MODE_FWD| S27_HEADER_EN));
#endif
...
#if defined(V54_BSP)
    /* max_frame_size */
    DPRINTF("S27_GLOBAL_CTRL_REG :%x\n", athrs27_reg_read(S27_GLOBAL_CTRL_REG));
    athrs27_reg_write(S27_GLOBAL_CTRL_REG,((athrs27_reg_read(S27_GLOBAL_CTRL_REG) & ~S27_MAX_FRAME_SIZE_MASK) |
    (0x600)));
    DPRINTF("S27_GLOBAL_CTRL_REG :%x\n", athrs27_reg_read(S27_GLOBAL_CTRL_REG));
#endif
...
#if defined(V54_BSP)
    /* 1 - Turn bit 14th of LEARN_EN OFF 
     * 2 - Turn bit 6th of PORT_LOCK_EN ON
     * 3 - Turn bit 5th of LOCK_DROP_EN OFF
     * Thus, we can avoid ingress package flooding to other ports.
     */
    athrs27_reg_write(S27_PORT_CONTROL_REGISTER1, S27_PORT_CONTROL_DEFAULT);
    athrs27_reg_write(S27_PORT_CONTROL_REGISTER2, S27_PORT_CONTROL_DEFAULT);
    athrs27_reg_write(S27_PORT_CONTROL_REGISTER3, S27_PORT_CONTROL_DEFAULT);
    athrs27_reg_write(S27_PORT_CONTROL_REGISTER4, S27_PORT_CONTROL_DEFAULT);
#endif
...
/******************************************************************************
*
* athrs27_phy_restart - restart auto-negotiation on the phy
*
* Re-start auto-negotiation on the phy
*/
void athrs27_phy_restart(int phyUnit)
{
    s27_wr_phy(1, phyUnit, ATHR_PHY_CONTROL, (s27_rd_phy(1, phyUnit, ATHR_PHY_CONTROL) | 0x200));
}
#endif
