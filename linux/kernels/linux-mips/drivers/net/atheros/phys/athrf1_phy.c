...
{ 
    ...
    if(!is_emu()) {
       ...
       printk("ATHR_AUTONEG_ADVERT:%X\n",phy_reg_read(ethUnit, ATHR_PHYADDR(phyUnit),ATHR_AUTONEG_ADVERT));
       printk("ATHR_1000BASET_CONTROL:%X\n",phy_reg_read(ethUnit, ATHR_PHYADDR(phyUnit),ATHR_1000BASET_CONTROL));
       printk("ATHR_PHY_CONTROL:%X\n",phy_reg_read(ethUnit, ATHR_PHYADDR(phyUnit),ATHR_PHY_CONTROL));
       ...
       phy_reg_write(ethUnit, ATHR_PHYADDR(phyUnit) , ATHR_1000BASET_CONTROL,0x0);
       phy_reg_write(ethUnit, ATHR_PHYADDR(phyUnit) , ATHR_PHY_CONTROL, ATHR_CTRL_AUTONEGOTIATION_ENABLE | ATHR_CTRL_SOFTWARE_RESET);
    }
...
#if defined(V54_BSP)    	
    return (phy_reg_read(ethUnit, ATHR_PHYADDR(phy_addr), reg_addr));
#else
...
void athrf1_wr_phy(int ethUnit,unsigned int phy_addr, unsigned int reg_addr, unsigned int write_data)
{
    ...
    if (is_ar934x()) ethUnit = 0;
#if defined(V54_BSP)    	
    phy_reg_write(ethUnit, ATHR_PHYADDR(phy_addr), reg_addr, write_data);
#else
...
#if defined(V54_BSP)
void athrf1_toggle_led(int status)
{
    if (!status)
        athrf1_wr_phy(0, 0, ATHR_PHY_LED_CONTROL, ATHR_STATUS_LED_OFF);
    else
        athrf1_wr_phy(0, 0, ATHR_PHY_LED_CONTROL, ATHR_STATUS_LED_ON);
}
#endif
