...
athr_gmac_ctrl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    switch (cmd) {
        ...
        case ATHR_FRAME_SIZE_CTL:
            ...
            break;
#if defined(V54_BSP)
        case ATHR_GMAC_PHY_RD:
            if (ethcfg->portnum != 0x3f) {
                ethcfg->val = phy->read_phy_reg(0,
                        ethcfg->portnum, ethcfg->phy_reg);
            }
            break;
        case ATHR_GMAC_PHY_WR:
            if (ethcfg->portnum != 0x3f) {
                phy->write_phy_reg(0,ethcfg->portnum,
                        ethcfg->phy_reg, ethcfg->val);
            }
            break;
        case ATHR_GMAC_PHY_FORCE:
            if (ethcfg->val == 10) {
                printk("Forcing 10Mbps %s on port:%d \n",
                        dup_str[ethcfg->duplex], (ethcfg->portnum));
                athr_forcephy_10M(dev, 0, ethcfg->portnum, ethcfg->duplex);
            } else if (ethcfg->val == 100) {
                printk("Forcing 100Mbps %s on port:%d \n",
                        dup_str[ethcfg->duplex], (ethcfg->portnum));
                athr_forcephy_100M(dev, 0, ethcfg->portnum, ethcfg->duplex);
            } else if (ethcfg->val == 0) {
            ...
            printk("Phy status:%x\n", 
                    phy->read_phy_reg(0, ethcfg->portnum, ATHR_PHY_CONTROL));
            mac->forced_phy_speed = ethcfg->val;
            break;
#endif             
        default:
             break;
