...
extern int athr_forcephy_10M(struct net_device *dev, int phyBase, int phy_addr, int duplex);
extern int athr_forcephy_100M(struct net_device *dev,int phyBase,  int phy_addr, int duplex);
#endif
...
#define ATHR_AP123_GPIO_SWAP    0x08000000
...
typedef struct sw_stats {
    ...
    uint32_t stat_tx_int;   /* isr athr_gmac_INTR_TX*/
    uint32_t stat_tx_be;    /* isr athr_gmac_INTR_TX_BUS_ERROR */
    uint32_t stat_tx_full;  /* same as tx_fifo_errors*/
    uint32_t stat_rx;       /* isr athr_gmac_INTR_RX */
    uint32_t stat_rx_ovf;   /* ist athr_gmac_INTR_RX_OVF */
    uint32_t stat_rx_be;    /* isr athr_gmac_INTR_RX_BUS_ERROR */
    uint32_t stat_rx_full;  /* poll athr_gmac_RX_STATUS_OOM */
    uint32_t stat_tx_timeout;
    uint32_t stat_dma_reset;
    uint32_t stat_dma_owned;
} sw_stats_t;
#endif
...
#endif
#if defined(CONFIG_MACH_AR934x) || defined(CONFIG_MACH_AR934x_MODULE)
...
#if defined(V54_BSP)
    /*  We let driver take care of which one is GE or FE. */
    extern athr_gmac_t *athr_gmacs[];
    if (cmd == ATHR_GMAC_CTRL_IOC && ethcfg->cmd == 0) {
        athr_gmac_t *mac = athr_gmacs[0];
        if(!strncmp(mac->mac_dev->name, dev->name, strlen(dev->name))) {
            cmd = ATHR_GMAC_CTRL_IOC;
            ethcfg->cmd = ATHR_GMAC_PHY_FORCE;
        } else {
            cmd = ATHR_PHY_CTRL_IOC;
            ethcfg->cmd = ATHR_PHY_FORCE;
        }
    }
#endif
    switch (cmd){
