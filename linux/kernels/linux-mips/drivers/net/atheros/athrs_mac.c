...
int walle2_ethname_remap[] = {4, 0, 1, 2, 3};
...
static inline int venet_enable(athr_gmac_t *mac)
{
    return (rks_is_walle2_board() && mac->mac_unit && venetEnabled);
}
...
athr_gmac_open(struct net_device *dev)
{
    ...
    uint32_t mask;
#if defined(V54_BSP)
    int phyBase = 0, phy_addr = 0, duplex = 1;
    ...
    if (venet_enable(mac)) {
        ...
        if (mac->mac_open > 1) {
            ...
            athrs27_phy_restart(vdev_state->unit_on_MAC);
            dev->trans_start = jiffies;
            return 0;
        }
       mac->devIrq = mac->mac_dev = dev;
    }
...
#else
        if (rks_is_gd26_board() || rks_is_gd36_board() || rks_is_walle2_board()) {
#endif
            if (is_ar934x() && is_s16()) {
...
#if defined(V54_BSP)
    if (rks_is_walle2_board() && venet_enable(mac) && tx_timeout_reset) {
            int i;
            for (i=0; i<dev_per_mac; i++) {
                ...
                athrs27_phy_restart(i); /* restart all PHY to let it take effect */
            }
        tx_timeout_reset = 0;
    }

    if (mac->forced_phy_speed != ATHR_PHY_SPEED_UNKNOWN)
        switch (mac->forced_phy_speed) {
            case 10:
            ...
            case 100:
                ...
                break;
            case 0:
                mac->phy->write_phy_reg(phyBase, phy_addr,
...
#else
    if (is_ar934x() && (is_f1e() || is_lantiq() || is_vir_phy()) && (mac == athr_gmacs[0])) {
#endif
        if (is_vir_phy())
            printk("WASP ----> VIR PHY\n");
        else
...
athr_gmac_hw_setup(athr_gmac_t *mac)
{
    ...
    if ((mac->mac_unit == 1)) {
        if (venet_enable(mac)) {
            athr_gmac_reg_rmw_set(mac, ATHR_GMAC_CFG2, (ATHR_GMAC_CFG2_CRC_EN));
            athr_gmac_reg_rmw_clear(mac, ATHR_GMAC_CFG2, (ATHR_GMAC_CFG2_PAD_CRC_EN |
    ...
    if (mac_has_flag(mac,ATHR_TX_FLCTL)) {
         ...
         */
        if (mac->mac_unit == 0 && ((is_s27() || is_s26()) || is_f1e() || is_lantiq())) {
...
athr_gmac_set_mac_from_link(athr_gmac_t *mac, athr_phy_speed_t speed, int fdx)
{
    ...
    case ATHR_PHY_SPEED_1000T:
	   ...
	   ops->set_pll(mac,mac->mac_speed);
        ...
        if (is_ar934x() && mac->mac_unit == 0 && (is_f1e() || is_lantiq()) )
...
#if 1
            if (is_lantiq()) {
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (2 << ATHR_GMAC_ETH_CFG_RXD_DELAY_SHIFT));
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (2 << ATHR_GMAC_ETH_CFG_RDV_DELAY_SHIFT));
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (2 << ATHR_GMAC_ETH_CFG_TXD_DELAY_SHIFT));
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (2 << ATHR_GMAC_ETH_CFG_TEN_DELAY_SHIFT));
            } else
#endif
            {
	...
	if (is_emu() && is_f1e()) {
	   ...
	   ops->set_pll(mac,mac->mac_speed);
        ...
        if ((is_ar7242() || is_ar934x()) && 
                    mac->mac_unit == 0 && (is_s16() || is_f1e() || is_lantiq()))
            athr_reg_wr(ATHR_GMAC_XMII_CONFIG,0x0101);

        if (is_ar934x() && mac->mac_unit == 0 && (is_f1e() || is_lantiq())) {
#if 1
            if (is_lantiq()) {
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (3 << ATHR_GMAC_ETH_CFG_RXD_DELAY_SHIFT));
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (3 << ATHR_GMAC_ETH_CFG_RDV_DELAY_SHIFT));
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (3 << ATHR_GMAC_ETH_CFG_TXD_DELAY_SHIFT));
                athr_reg_rmw_set(ATHR_GMAC_ETH_CFG, (3 << ATHR_GMAC_ETH_CFG_TEN_DELAY_SHIFT));
            } else
#endif
            {
...
athr_gmac_check_link(void *arg,int phyUnit)
{
...
#if defined(V54_BSP)
    if (venet_enable(mac))
        carrier = netif_carrier_ok(mac->vdev_state[phyUnit].dev);
    ...
    phy_up &= 0xf;
    {
        int i=4, total=0;
        while (i) {
            ((phy_up >> (i-1)) & 1) ? total++ : 0;
            i--;
        }
        printk("phy_up(0x%x => 0x%x)\n", phy_up, total);
        phy_up2 = phy_up;
        phy_up = total;
    }
    if (phy_up)
...
#endif
    {
        if (carrier)
        {
            ...
            if (venet_enable(mac)) {
                ...
                printk("athr_gmac link: vdev %x down\n",
    ...
    if (phy->is_alive(phyUnit) || is_ar9330_emu())
    {
        ...
        */
        ...
        if (venet_enable(mac)) {
            ...
            printk("athr_gmac: vdev %x up, speed(%d) fdx(%d)\n",
                    (unsigned int)mac->vdev_state[phyUnit].dev,
                    speed, fdx);
        } else {
...
#if defined(V54_BSP)
    if (venet_enable(mac)) {
        /* Skip this check due to internal switch */
    } else {
...
#if !defined(V54_BSP)    
        /* check this at athr_gmac_hard_start */
...
static void
athr_gmac_add_hdr(athr_gmac_t *mac,struct sk_buff *skb,int unit_on_MAC)
{
    int ac = skb->priority;
    ...
    else if (mac_has_flag(mac,ATHR_S27_HEADER))
    {
        ...
        memcpy(skb->data,skb->data + ATHR_HEADER_LEN, 12);
#if defined(V54_BSP)
        skb->data[13] = (0x01 << (unit_on_MAC + 1));
#else
...
athr_gmac_hard_start(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (venet_enable(mac)) {
        ...
        if (headroom < ATHR_HEADER_LEN) {
            headroom += ATHR_HEADER_LEN;
            alloc_skb++;
        }
    ...
    if (mac_has_flag(mac, ATHR_HEADER_ENABLED))
        athr_gmac_add_hdr(mac, skb, vdev_state->unit_on_MAC);
...
athr_gmac_intr(ATHR_MAC_ISR_ARGS)
{
    ...
    if (isr & (ATHR_GMAC_INTR_RX_OVF))
    {
        ...
        if (is_ar934x()) {
            ...
            } else if (mac_has_flag(mac,ATHR_RX_POLL)) {
...
process_pkts:
    ...
    while(quota)
    {
        ...
        if (athr_gmac_rx_owned_by_dma(ds))
        {
            ...
            if (quota == iquota) {
#if 0
                printk("%s: %d: %d: quota(%d) mask(%x) status(%x) head(%d) tail(%d)\n",
                    __func__, __LINE__, (last_dma_owned+1), quota,
                    athr_gmac_reg_rd(mac, ATHR_GMAC_DMA_INTR_MASK),
                    status, r->ring_head, r->ring_tail);
#endif
...
#if defined(V54_BSP)
        else if (last_dma_owned) {
#if 0
            printk("%s: %d: last_dma_owned: quota(%d) mask(%x) status(%x) head(%d) tail(%d)\n",
                __func__, __LINE__, quota,
                athr_gmac_reg_rd(mac, ATHR_GMAC_DMA_INTR_MASK),
                status, r->ring_head, r->ring_tail);
#endif
            last_dma_owned = 0;
        ...
        if (venet_enable(mac)) {
            ...
            srcPort = (*(u8 *)(skb->data + (ETH_ALEN * 2) + 1) & 0x0f) - 1;
            memcpy(dst_src_addr, skb->data, (ETH_ALEN * 2));
            memcpy((skb->data + ATHR_HEADER_LEN), dst_src_addr, (ETH_ALEN * 2));
            skb->data += ATHR_HEADER_LEN;
            skb->tail += ATHR_HEADER_LEN;
            len       -= ATHR_HEADER_LEN;
...
athr_gmac_tx_timeout(struct net_device *dev)
{
    ...
    if (rks_is_walle2_board()) {
        athr_gmac_int_disable(mac);
        athrs27_disable_link_intrs(mac);
        athr_gmac_rx_stop(mac);
        athr_gmac_tx_stop(mac);
...
#if defined(V54_BSP)
    if (mac == athr_gmacs[1]) {
        athr_gmac_stop(mac->mac_dev);
        athr_gmac_open(mac->mac_dev);
    }
#endif
}
    ...
    *athr_gmac_get_stats(struct net_device *dev)
{
    ...
    if (venet_enable(mac)) {
        ...
        if (vdev_state)
            ...
            printk("athr_gmac cannot find vdev stats\n");
...
athr_gmac_init(void)
{
    ...
    if ((rks_is_gd36_board() || rks_is_gd26_board()) && !rks_is_r300_board()) {
        ethname_remap = &gd36_ethname_remap[0];
    }else if (rks_is_walle2_board()) {
        ethname_remap = &walle2_ethname_remap[0];
    }
#endif
...
#else
    if (rks_is_walle2_board() && (totalDev > 2)) {
        venetEnabled = 1;
        dev_per_mac = totalDev - 1;
    }
    if (venetEnabled) printk("%s: venetEnabled = %d dev_per_mac = %d\n", __func__, venetEnabled, dev_per_mac);
    for(i = 0; i < ((totalDev < ATHR_GMAC_NMACS) ? totalDev : ATHR_GMAC_NMACS); i++)
    {
#endif
        if ((rks_is_spaniel_board() || rks_is_r300_board()) & i) break;
        printk("%s\n", ethname_array[ethname_remap[ethnameIdx]]);
#if !defined(V54_BSP)
        dev = alloc_netdev(ATHR_MAC_ETHERDEV_SZ, ethname_array[ethname_remap[ethnameIdx++]], ether_setup);
#else
        ...
        mac->forced_phy_speed = ATHR_PHY_SPEED_UNKNOWN;
        *pmac = mac;
#endif
        ...
        */
        ...
        if (venet_enable(mac)) {
            ...
            if (!mac->vdev_state) {
                printk("athr_gmac cannot allocate vdev_state\n");
                goto failed;
            } else {
        ...
        if (venet_enable(mac)) {
            ...
            for (unitOnMac = 1; unitOnMac < dev_per_mac; unitOnMac++) {
                ...
                vdev->netdev_ops      = (const struct net_device_ops *)&athr_gmac_net_ops;
                pmac = (athr_gmac_t **)ATHR_MAC_PRIV(vdev);
                *pmac                 =  mac;
                ...
                if (!bd_get_lan_macaddr(nextDev, vdev->dev_addr)) {
                    athr_gmac_get_default_macaddr(mac, vdev->dev_addr);
                } else {
                    int addr;
                ...
                if (register_netdev(vdev)) {
                    printk("athr_gmac register vdev failed\n");
                    goto failed;
                }
                nextDev++;
            }
...
#if defined(V54_BSP)
            if (venet_enable(athr_gmacs[i])) {
                int unitOnMac;
                for (unitOnMac = 1; unitOnMac < dev_per_mac; unitOnMac++) {
                    if (athr_gmacs[i]->vdev_state[unitOnMac].dev)
                        free_netdev(athr_gmacs[i]->vdev_state[unitOnMac].dev);
                }
                if (athr_gmacs[i]->vdev_state)
                    kfree(athr_gmacs[i]->vdev_state);
            }
#endif
    ...
    for(i = 0; i < ATHR_GMAC_NMACS; i++)
    {
        ...
        if (venet_enable(athr_gmacs[i])) {
            ...
            for (unitOnMac = 0; unitOnMac < dev_per_mac; unitOnMac++) {
                if (athr_gmacs[i]->vdev_state[unitOnMac].dev) {
                    if (athr_gmacs[i]->devIrq == athr_gmacs[i]->vdev_state[unitOnMac].dev)
                        continue;
#if VENETDEV_DEBUG
                    printk("cleanup: free dev=0x%x\n", (u32)athr_gmacs[i]->vdev_state[unitOnMac].dev);
#endif
                    unregister_netdev(athr_gmacs[i]->vdev_state[unitOnMac].dev);
                    free_netdev(athr_gmacs[i]->vdev_state[unitOnMac].dev);
                }
            }
#if VENETDEV_DEBUG
            printk("cleanup: free devIrq=0x%x\n", (u32)athr_gmacs[i]->devIrq);
#endif
            if (athr_gmacs[i]->devIrq) {
                unregister_netdev(athr_gmacs[i]->devIrq);
                free_netdev(athr_gmacs[i]->devIrq);
            }
            if (athr_gmacs[i]->vdev_state)
                kfree(athr_gmacs[i]->vdev_state);
        } else {
