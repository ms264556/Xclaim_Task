...
static void sierra_net_printk_skb(struct sk_buff *skb)
{
#ifdef VERBOSE
#define BYTES_PER_LINE  16
	...
	printk(KERN_DEBUG "skb=%p, head=%p, data=%p, tail=%p, end=%p, "
	       "headroom=%d, tailroom=%d, len=%d\n",
	       skb, skb->head, skb->data, skb->tail, skb->end,
	       skb_headroom(skb), skb_tailroom(skb), skb->len);

	for (i = 0, byte=0; 
	     i < (skb->len + BYTES_PER_LINE - 1) / BYTES_PER_LINE;
	     i++)
	{
		ptr = strbuf;
		for (k = 0; (k < BYTES_PER_LINE) && (byte < skb->len); k++, byte++) {
...
static int sierra_net_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	/* Ruckus: Endian fix from OpenWrt patch at
	 * https://dev.openwrt.org/browser/trunk/package/sierra-directip/patches
	 */
 ...
 */ 
	...
	if (!(status == sizeof(fwattr) && (cpu_to_le16(fwattr) & SWI_GET_FW_ATTR_MASK))) {
		...
		return -ENODEV;
#else
		/* Ruckus: DHCP requirement fix from OpenWrt patch at
		 * https://dev.openwrt.org/browser/trunk/package/sierra-directip/patches
		 */
...
/* ---------------------------- Transmit data path ----------------------*/
static void sierra_net_tx_reserve_headroom(struct usbnet *dev, struct sk_buff **pskb,
		gfp_t flags)
{
	struct sk_buff *skb;
	...
	/* If headroom is big enough to hold HIP header, do nothing */
	headroom = skb_headroom(skb);
	if (SIERRA_NET_HIP_EXT_HDR_LEN <= headroom)
		return;

	/* If skb is not cloned and tailroom could be used partially, use it.
	 * Othereise copy expand skb */
	tailroom = skb_tailroom(skb);
	if ((!skb_cloned(skb)) && 
            (SIERRA_NET_HIP_EXT_HDR_LEN - headroom <= tailroom)) {
		skb->data = memmove(skb->head + SIERRA_NET_HIP_EXT_HDR_LEN,
		                    skb->data, skb->len);
		skb_set_tail_pointer(skb, skb->len);
		dev_dbg(&dev->udev->dev, "%s headroom=%d, tailroom=%d, "
		        "skb memmove to hold %d HIP_EXT_HDR", 
			__func__, headroom, tailroom, SIERRA_NET_HIP_EXT_HDR_LEN);
	} else {
		struct sk_buff *skb2;
		/* New headroom and tailroom both 16 bytes */
		skb2 = skb_copy_expand(skb, 16, 16, flags);
		dev_kfree_skb_any(skb);
		*pskb = skb2;
		dev_dbg(&dev->udev->dev, "%s headroom=%d, tailroom=%d, "
		        "cloned skb copy_expand to hold %d HIP_EXT_HDR", 
			__func__, headroom, tailroom, SIERRA_NET_HIP_EXT_HDR_LEN);
	}
}

static char *sierra_net_device_ip_address(struct net_device *net)
{
	struct in_device *ip;
	struct in_ifaddr *in;
	...
	if (!in)
		return NULL;
	return (char *)&in->ifa_address; 
}

static int sierra_net_tx_proxy_arp(struct usbnet *dev, struct sk_buff *skb)
{
	struct arp {
		struct arphdr ar_hdr;           /* ARP header */
	...
	/*
	 * This fucntion implements "Proxy ARP", since the northbound LTE network
	 * will not answer ARP requests. When the USB interface is a member of
	 * a bridge interface, interface NOARP flag is not enough.
	 * FIXME: VLAN case?
	 */

	/* Get Ethernet and ARP header pointers */
	eth = (struct ethhdr *)skb->data;
	...
	dev_dbg(&dev->udev->dev, "%s(), ETH dest MAC="FMT_MAC", src MAC="FMT_MAC
	        ", proto=0x%04x, ARP opcode=%d"
	        ", sender MAC="FMT_MAC", sender IP="NIPQUAD_FMT
	        ", target MAC="FMT_MAC", target IP="NIPQUAD_FMT"\n", 
	        __func__, ARG_MAC(eth->h_dest), ARG_MAC(eth->h_source),
	        eth->h_proto, arp->ar_hdr.ar_op,
	        ARG_MAC(arp->ar_sha), NIPQUAD(arp->ar_sip),
	        ARG_MAC(arp->ar_tha), NIPQUAD(arp->ar_tip));

	/* Only care about ARP REQUEST */
	if (arp->ar_hdr.ar_op != ntohs(ARPOP_REQUEST))
		goto DUMP_IT;

	/* Try get local IP address: 
	 * 1) the USB network interface first;
	 * 2) the bridge where the USB network interface is a memeber second
	 * If neither got IP address ready, return
	 */
	if (!dev || !dev->net)
		goto DUMP_IT;
	net = dev->net;
	ip_addr = sierra_net_device_ip_address(net);
	if (!ip_addr) {
		br_net = br_net_device(net);
		ip_addr = sierra_net_device_ip_address(br_net);
		if (!ip_addr)
			goto DUMP_IT;
	}
 
	/* Ethernet dest MAC is the original source;
	 * Ethernet source MAC is local MAC address.
	 */
	memcpy(eth->h_dest, eth->h_source, ETH_ALEN);
	memcpy(eth->h_source, dev->net->dev_addr, ETH_ALEN);

	/* ARP header: op code is ARP REPLY */
	arp->ar_hdr.ar_op = ntohs(ARPOP_REPLY);

	/* ARP body:
	 * target hardware address: original sender hardware address 
	 * target IP address: original sender IP address
	 * sender hardware address: local MAC address
	 * sender IP address: local IP address
	 */
	memcpy(arp->ar_tha, arp->ar_sha, ETH_ALEN);
	memcpy(arp->ar_tip, arp->ar_sip, 4);
	memcpy(arp->ar_sha, dev->net->dev_addr, ETH_ALEN);
	memcpy(arp->ar_sip, ip_addr, 4);

	/* Now let the boomerang return */
		...
		gfp_t flags)
{
	...
	/* If ARP REQUEST, proxy it via a local ARP REPLY */
	if (sierra_net_tx_proxy_arp(dev, skb)) {
		return NULL;
	}

	dev_dbg(&dev->udev->dev, "%s", __func__);
	if (priv->link_up && check_ethip_packet(skb, dev) && is_ip(skb)) {
		/* Ruckus: reserve enough head room */
		sierra_net_tx_reserve_headroom(dev, &skb, flags);
		if (!skb)
			return NULL;
