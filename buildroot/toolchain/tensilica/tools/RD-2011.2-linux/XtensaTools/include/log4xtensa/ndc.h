...
// 2011.06.02.  Tensilica.  Move definition of DiagnosticContext to avoid EDG parser error (Synopsys F-2011.06-Beta)
// 2006.06.29.  Tensilica.  Replace LO4CPLUS (note misspelling) with LOG4XTENSA
...
#ifndef _LOG4XTENSA_NDC_HEADER_
#define _LOG4XTENSA_NDC_HEADER_
...
namespace log4xtensa {
     ...
     */
    struct LOG4XTENSA_EXPORT DiagnosticContext {
      // Ctors
        DiagnosticContext(const log4xtensa::tstring& message, DiagnosticContext *parent);
        DiagnosticContext(const log4xtensa::tstring& message);

      // Data
        log4xtensa::tstring message; /*!< The message at this context level. */
        log4xtensa::tstring fullMessage; /*!< The entire message stack. */
    };
     ...
     */
    class LOG4XTENSA_EXPORT NDC : protected log4xtensa::helpers::LogLogUser {
    public:
        /**
         ...
         * @see NDCContextCreator
         */
        void push(const log4xtensa::tstring& message);
    ...
    private:
      // Methods
        DiagnosticContextStack* getPtr()
            { return static_cast<DiagnosticContextStack*>
                          (LOG4XTENSA_GET_THREAD_LOCAL_VALUE( threadLocal )); }

      // Data
        LOG4XTENSA_THREAD_LOCAL_TYPE threadLocal;
      ...
      // Friends
        friend LOG4XTENSA_EXPORT NDC& getNDC();
    };
     ...
     */
    class LOG4XTENSA_EXPORT NDCContextCreator {
    public:
        /** Pushes <code>msg</code> onto the NDC stack. */
        NDCContextCreator(const log4xtensa::tstring& msg) { getNDC().push(msg); }
