#ifndef _XTSC_QUEUE_POP_IF_SD_MON_H_
#define _XTSC_QUEUE_POP_IF_SD_MON_H_
...
class XTSC_SD_API xtsc_queue_pop_if_sd_mon : public sc_mx_import_module, public MxDIBase, public MxPI {
public:
  sc_export<xtsc::xtsc_queue_pop_if>            m_queue_pop_export;
  sc_port<xtsc::xtsc_queue_pop_if>              m_queue_pop_port;
      
  SC_HAS_PROCESS(xtsc_queue_pop_if_sd_mon);

  // constructor / destructor
  xtsc_queue_pop_if_sd_mon(sc_mx_m_base* c, const sc_module_name &module_name);
  virtual ~xtsc_queue_pop_if_sd_mon();

  // Thread to count notifications of the nonempty event; obtained from default_event()
...
protected:
  ...
  class xtsc_queue_pop_if_impl : public xtsc::xtsc_queue_pop_if, public sc_object {
  public:

    /// Constructor
    xtsc_queue_pop_if_impl(const char *object_name, xtsc_queue_pop_if_sd_mon& monitor) :
  ...
  protected:
    ...
    xtsc_queue_pop_if_sd_mon&   m_monitor;       ///< Our xtsc_queue_pop_if_sd_mon object
...
private:
  ...
  xtsc::u64                             m_num_nb_can_pop_true;          ///< Total calls to nb_can_pop that returned true
  xtsc::u64                             m_num_nb_can_pop_false;         ///< Total calls to nb_can_pop that returned false
  xtsc::u64                             m_num_nb_pop_true;              ///< Total calls to nb_pop that returned true
  xtsc::u64                             m_num_nb_pop_false;             ///< Total calls to nb_pop that returned false
  xtsc::u64                             m_num_nonempty_events;          ///< Total times the nonempty event has been notified

  xtsc::u64                             m_ticket;                       ///< Last queue ticket
  sc_unsigned                          *m_p_element;                    ///< Last element poped

  // MxPI
  MxPIChannelSymbolInfo_t               m_bool_symbols;                 ///< Symbols for true and false 
  MxPIStreamInfo_t                      m_nb_can_pop_stream_info;       ///< MxPI stream info
  MxPIStream_t                         *m_p_nb_can_pop_stream;
  MxPIChannel_t                        *m_nb_can_pop_channel_ptr_tab[1];
  MxPIChannel_t                         m_nb_can_pop_return_channel;
  MxPIStreamInfo_t                      m_nb_pop_stream_info;           ///< MxPI stream info
  MxPIStream_t                         *m_p_nb_pop_stream;
  MxPIChannel_t                        *m_nb_pop_channel_ptr_tab[1];
  MxPIChannel_t                         m_nb_pop_return_channel;
...
#endif  // _XTSC_QUEUE_POP_IF_SD_MON_H_
