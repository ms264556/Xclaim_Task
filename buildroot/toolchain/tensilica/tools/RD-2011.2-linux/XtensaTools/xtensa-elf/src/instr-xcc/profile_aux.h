...
// $Revision: 1.10 $
// $Date: 2001/03/12 18:39:26 $
// $Author: mtibuild $
// $Source: /isms/cmplrs.src/osprey1.0/instrumentation/libinstr/RCS/profile_aux.h,v $
//
// ====================================================================
//
...
#ifdef  FB_64BIT_COUNTERS
  #define FREQ_INT_TYPE INT64
#else
  #ifdef FB_32BIT_COUNTERS
    #define FREQ_INT_TYPE UINT32
  #else
    #pragma error "Undefined FREQ_INT_TYPE"
  #endif
#endif
...
struct Compgoto_Profile {
    typedef DYN_ARRAY<FREQ_INT_TYPE> value_type;
...
struct Short_Circuit_Profile {
   FREQ_INT_TYPE right_taken_count;
   FREQ_INT_TYPE neither_taken_count;
...
struct Call_Profile {
   FREQ_INT_TYPE entry_count;
   FREQ_INT_TYPE exit_count;

   Call_Profile() : entry_count(0), exit_count(0) {}
};

// All the information about a RETURN is stored in
// Return_Profile structure.

struct Return_Profile {
   FREQ_INT_TYPE return_count;

   Return_Profile() : return_count(0) {}
};
...
struct Loop_Profile {
   FREQ_INT_TYPE invocation_count; // total times the loop is invoked
   FREQ_INT_TYPE total_trip_count; // total trip count from all invocations
   FREQ_INT_TYPE last_trip_count;  // trip count from last invocation
#ifdef __NOT_USED__
   FREQ_INT_TYPE min_trip_count; // minimum trip count from previous invocations
   FREQ_INT_TYPE max_trip_count; // maximum trip count from previous invocations
#endif
   FREQ_INT_TYPE num_zero_trips;   // Number of times the loop had a zero trip

   Loop_Profile () :
       invocation_count(0), total_trip_count(0), last_trip_count(0),
#ifdef __NOT_USED__
       min_trip_count(0), max_trip_count(0), 
#endif
       num_zero_trips(0) {}
};
...
struct Switch_Profile {
    typedef DYN_ARRAY<FREQ_INT_TYPE> value_type;
    typedef DYN_ARRAY<INT64> case_value_type;
    value_type targets_profile;
    case_value_type targets_case_value;
    Switch_Profile() {}
    ...
    case_value_type& Get_Targets_Case_Value() {
...
typedef DYN_ARRAY<Invoke_Profile>	Invoke_Profile_Vector;
typedef DYN_ARRAY<Branch_Profile>	Branch_Profile_Vector;
typedef DYN_ARRAY<Switch_Profile>	Switch_Profile_Vector;
typedef DYN_ARRAY<Compgoto_Profile>	Compgoto_Profile_Vector;
typedef DYN_ARRAY<Loop_Profile>		Loop_Profile_Vector;
typedef DYN_ARRAY<Short_Circuit_Profile>  Short_Circuit_Profile_Vector;
typedef DYN_ARRAY<Call_Profile>		Call_Profile_Vector;
typedef DYN_ARRAY<Return_Profile>       Return_Profile_Vector;

class PU_Profile_Handle {
public:
    ...
    Return_Profile_Vector       Return_Profile_Table;
    ...
    PU_Profile_Handle() : cycle_count(0),checksum(0), opt_flags(0) { inited = false;}
    
    PU_Profile_Handle(char *fname, char *pname, INT32 c_sum, INT32 o_flags) {
        cycle_count = 0;
	opt_flags = o_flags;
	checksum = c_sum;
    ...
    Return_Profile_Vector& Get_Return_Table () {
        return Return_Profile_Table;
    }
...
typedef HASH_TABLE<long, PU_PROFILE_HANDLE > HASH_MAP;
typedef HASH_TABLE_ITER< long, PU_PROFILE_HANDLE > HASH_MAP_ITER;
