/* This is a modified copy of mallocr.c from newlib.
...
/* Ferret: Build everything in one object.  */
#undef SEPARATE_OBJECTS
#define HAVE_MMAP 0
...
#define chunk2mem(p)   ((Void_t*)((char*)(p) + 2*SIZE_SZ + GUARD_ZONE_SIZE))
#define mem2chunk(mem) ((mchunkptr)((char*)(mem) - 2*SIZE_SZ - GUARD_ZONE_SIZE))

/* pad request bytes into a usable size */

#define request2size(req) \
 (((unsigned long)((req) + (2 * GUARD_ZONE_SIZE) + (SIZE_SZ + MALLOC_ALIGN_MASK)) < \
  (unsigned long)(MINSIZE + MALLOC_ALIGN_MASK)) ? ((MINSIZE + MALLOC_ALIGN_MASK) & ~(MALLOC_ALIGN_MASK)) : \
   (((req) + (2 * GUARD_ZONE_SIZE) + (SIZE_SZ + MALLOC_ALIGN_MASK)) & ~(MALLOC_ALIGN_MASK)))
  ...
  else
  {
    ...
    if (old_top != initial_top)
    {
      ...
      chunk_at_offset(old_top, old_top_size + SIZE_SZ)->size =
        SIZE_SZ|PREV_INUSE;
      /* Skip this to avoid confusing ferret.  */
#if 0
...
#endif
{
  ...
  {
    __simc_no_heap (&_heap_sentry, bytes);
    errno = ENOMEM;
    return 0;
  }
  ...
  if (is_small_request(nb))  /* Faster version for small requests */
  {
    ...
    if (victim != q)
    {
      ...
      MALLOC_UNLOCK;
      __simc_leave_ferret ();
      __simc_malloc (chunk2mem(victim), bytes);
...
#endif
{
  ...
  INTERNAL_SIZE_T    oldsize; /* 'oldp' size */

  Void_t*   newmem;           /* newly allocaetd user mem */
  mchunkptr newp;             /* chunk corresponding to newmem */
  INTERNAL_SIZE_T    newsize; /* 'newp' size */
  INTERNAL_SIZE_T    cpsize; /* 'newp' size */
...
#if HAVE_MMAP
#error ferret-realloc does not support MMAP
#endif
  ...
  /* Allocate new memory region, we always do this even if memory is shrinking... */
  ...
  /* We want to copy the user portion of the chunk (so skip the two
     SIZE_SZ fields at the beginning). */
  cpsize = (oldsize > newsize) ? newsize : oldsize;
  __simc_enter_ferret ();
  memcpy(((char *)newp) + (2 * SIZE_SZ), ((char *)oldp) + (2 * SIZE_SZ), cpsize - (2* SIZE_SZ));
  __simc_leave_ferret ();
  fREe(RCALL oldmem);
  MALLOC_UNLOCK;
  return newmem;
...
#endif
{
  ...
  else /* misaligned */
  {
    ...
    */
    ...
    /* Ferret thinks the whole thing is now freed -- allocate it again.  */
    __simc_malloc (chunk2mem(newp), bytes);
    p = newp;

    assert (newsize >= nb && (((unsigned long)(chunk2mem(p))) % alignment) == 0);
  }

  /* Ferret: Skip this.  It's good enough without it, even though it
     wastes memory and ferret will fail to warn about accesses beyond
     "bytes" if the result of the initial malloc was already
     aligned.  */
#if 0
...
#endif
{
  ...
  else
  {
    ...
    MALLOC_ZERO(mem - GUARD_ZONE_SIZE, csz - SIZE_SZ);
    __simc_leave_ferret ();
    return mem;
  }
}
