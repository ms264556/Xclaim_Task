...
// Module: fb_freq.h
// $Revision: 1.5 $
// $Date: 2000/04/06 02:25:36 $
// $Author: mtibuild $
// $Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/fb_freq.h,v $
//
// Description:
//
...
const float FB_FREQ_EPSILON = 0.01;
...
public:
  ...
  FB_FREQ( float value, bool exact )
    : _type( exact ? FB_FREQ_TYPE_EXACT : FB_FREQ_TYPE_GUESS ),
      _value( value )
    { Is_True( value >= (float)0.0, ( "FB_FREQ: negative value %f", value ) ); }
  ...
  friend bool operator== ( const FB_FREQ freq1, const FB_FREQ freq2 ) {
    // check for unknown values. Can't use the macros
    // because that results in a recursive call
    if ((freq1._value < 0) ||
	(freq2._value < 0))
      return false;
