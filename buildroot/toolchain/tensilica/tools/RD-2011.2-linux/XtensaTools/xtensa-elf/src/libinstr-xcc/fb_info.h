...
// Module: fb_info.h
// $Revision: 1.11 $
// $Date: 2000/05/31 19:42:40 $
// $Author: dlstephe $
// $Source: /isms/cmplrs.src/osprey1.0/common/com/RCS/fb_info.h,v $
//
// Description:
//
...
struct FB_Info_Loop {
  ...
  void Byteswap() { 
    freq_zero.Byteswap();
    freq_positive.Byteswap();
    freq_out.Byteswap();
    freq_back.Byteswap();
    freq_exit.Byteswap();
    freq_iterate.Byteswap();
  }
  ...
  void freq_verify() const {
    if (freq_zero + freq_out != freq_exit &&
	freq_positive.Known() && freq_exit.Known())
      DevWarn("Inconsistently annotated loop");
    if (freq_positive + freq_back != freq_iterate &&
	freq_positive.Known() && freq_back.Known())
      DevWarn("Inconsistenly annotated loop");
  }
...
struct FB_Info_Call {
  ...
  FB_Info_Call( FB_FREQ entry, FB_FREQ exit, BOOL same = FALSE )
    : freq_entry(  entry ),
       freq_exit(   exit  ),
       in_out_same( same ),
       dummy_buffer( 0 )  // And it must be initialized to avoid reading
                         // uninited memory in copy constructors
  {
...
struct FB_Info_Return {
  FB_FREQ freq_return;

  FB_Info_Return( FB_FREQ _return ) :
    freq_return( _return )
  {}

  FB_Info_Return() :
    freq_return(FB_FREQ_UNINIT) {}

  void Byteswap() { 
    freq_return.Byteswap();
  }

  void Print( FILE *fp ) const {
    fprintf( fp, "FB---> return = " );
    freq_return.Print( fp );
  }
};
...
struct FB_Info_Switch {
  ...
  FB_Info_Switch( const vector<FB_FREQ>::size_type n, 
                  FB_FREQ initial_freq = FB_FREQ_UNINIT )
    : freq_targets( vector<FB_FREQ>( n, initial_freq ) ) {}
  ...
  void Byteswap() { 
    for (vector<FB_FREQ>::iterator target_freq = freq_targets.begin(); 
         target_freq != freq_targets.end(); 
         ++target_freq)
      target_freq->Byteswap();
  }
...
typedef vector<FB_Info_Switch, mempool_allocator<FB_Info_Switch> >
					FB_Switch_Vector;
typedef vector<FB_Info_Return, mempool_allocator<FB_Info_Return> >
                                        FB_Return_Vector;
