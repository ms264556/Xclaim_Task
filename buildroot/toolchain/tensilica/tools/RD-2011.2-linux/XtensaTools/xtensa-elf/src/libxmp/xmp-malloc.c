...
/* TENSILICA WARNING: Many elements of this file are duplicated in
   xmp-shared-data.c and must be kept in sync.  We are trying to keep
   this file as close to newlib's mallocr.c as possible, so
   re-factoring is not an option.  */
...
#define HAVE_MMAP 0
extern void * _xmp_sbrk_r(struct _reent * reent_ptr, size_t size);
#define MORECORE(size) _xmp_sbrk_r(reent_ptr, (size))
...
/* Tensilica: copied into xmp-shared-data.c  */
...
#define cALLOc		_xmp_calloc_r
#define fREe		_xmp_free_r
#define mALLOc		_xmp_malloc_r
#define mEMALIGn	_xmp_memalign_r
#define rEALLOc		_xmp_realloc_r
#define vALLOc		_xmp_valloc_r
#define pvALLOc		_xmp_pvalloc_r
#define mALLINFo	_xmp_mallinfo_r
#define mALLOPt		_xmp_mallopt_r
...
#if __STD_C
Void_t* xmp_malloc(size_t bytes)
{
  return _xmp_malloc_r (_REENT, bytes);
}

#include "xmp-library.h"
extern xmp_mutex_t xmp_malloc_lock;

extern void __malloc_lock(struct _reent * r)
{
  xmp_mutex_lock(&xmp_malloc_lock);
}

extern void __malloc_unlock(struct _reent *r)
{
  xmp_mutex_unlock(&xmp_malloc_lock);
}
...
#if __STD_C
void xmp_free (Void_t * aptr)
{
  _xmp_free_r (_REENT, aptr);
}
