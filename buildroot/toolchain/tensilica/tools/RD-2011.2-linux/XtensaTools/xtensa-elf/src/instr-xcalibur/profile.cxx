...
PROFILE_PHASE instrumentation_phase_num = PROFILE_PHASE_NONE;
unsigned int instrumentation_flags = 0;
...
Profile_Invoke_Init(PU_PROFILE_HANDLE pu_handle, INT32 num_invokes)
{
  ...
  if (Inv_Table.empty()) {
    Inv_Table.reserve_and_bzero(num_invokes);
  }
}
...
Profile_Branch_Init(PU_PROFILE_HANDLE pu_handle, INT32 num_branches)
{
  ...
  if (Br_Table.empty()) {
    Br_Table.reserve_and_bzero(num_branches);
  }
}
		    ...
		    INT32 num_case_values, INT64 *case_values)
{
  ...
  if (Switch_Table.empty()) {
	...
	sp.targets_profile.reserve_and_bzero(num_targets + 1);
		      ...
		      INT32 *compgoto_num_targets)
{
  ...
  if (Compgoto_Table.empty()) {
	 ...
	 Compgoto_Table[i].Get_Targets_Profile().reserve_and_bzero(num_targets);
...
Profile_Loop_Init(PU_PROFILE_HANDLE pu_handle, INT32 num_loops)
{
  ...
  if (Loop_Table.empty()) {
     Loop_Table.reserve_and_bzero(num_loops);
  }
}
			   ...
			   INT32 num_short_circuit_ops)
{
  ...
  if (Short_Circuit_Table.empty()) {
     Short_Circuit_Table.reserve_and_bzero(num_short_circuit_ops);
  }
}
...
Profile_Call_Init(PU_PROFILE_HANDLE pu_handle, INT32 num_calls)
{
  ...
  if (Call_Table.empty()) {
     Call_Table.reserve_and_bzero(num_calls);
  }
}
