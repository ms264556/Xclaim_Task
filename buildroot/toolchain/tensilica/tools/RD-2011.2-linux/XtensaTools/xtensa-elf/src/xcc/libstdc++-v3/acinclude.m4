...
AC_DEFUN([GLIBCXX_CONFIGURE], [
  ...
  m4_define([glibcxx_SUBDIRS],[include libmath libsupc++ src po])
  ...
  # TENSILICA: Added --with-uclibc.
  AC_ARG_WITH([uclibc],
    AC_HELP_STRING([--with-uclibc],
                   [assume uclibc as a system C library]))

  # TENSILICA: Added --with-xclib.
  AC_ARG_WITH([xclib],
    AC_HELP_STRING([--with-xclib],
                   [assume xclib as a system C library]))
  ...
  AC_PROG_LN_S
  # TENSILICA: Avoid Cygwin symlinks.
  case `uname -s` in
  CYGWIN*)
    LN_S='cp -p' ;;
  esac
...
-I$glibcxx_srcdir/libsupc++"
  ...
  TOPLEVEL_INCLUDES='-I$(toplevel_srcdir)/gcc-tensilica'
  if test "x${with_xclib}" = "xyes"; then
    TOPLEVEL_INCLUDES=" \
      -isystem $toplevel_srcdir/include \
      -I$toplevel_srcdir/gcc-tensilica"
  else
    TOPLEVEL_INCLUDES='-I$(toplevel_srcdir)/gcc-tensilica'
  fi
...
AC_DEFUN([GLIBCXX_EXPORT_INSTALL_INFO], [
  ...
  if test x"$glibcxx_toolexecdir" = x"no"; then
    if test -n "$with_cross_host" &&
      ...
      # TENSILICA: Put XCC libraries in a subdirectory.
      glibcxx_toolexeclibdir=$glibcxx_toolexeclibdir/xcc
    else
      glibcxx_toolexecdir='${libdir}/gcc/${host_alias}'
