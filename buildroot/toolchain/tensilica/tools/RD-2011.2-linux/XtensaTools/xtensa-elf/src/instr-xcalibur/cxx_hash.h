...
// Module: cxx_hash.h
// $Revision: 1.11 $
// $Date: 2000/05/19 20:18:29 $
// $Author: murthy $
// $Source: /isms/cmplrs.src/osprey1.0/be/com/RCS/cxx_hash.h,v $
//
// Revision history:
...
#ifdef _KEEP_RCS_ID
static char *cxx_hash_rcs_id  = cxx_hash_INCLUDED "$Revision: 1.11 $";
#endif /* _KEEP_RCS_ID */
...
#ifdef USE_MALLOC
public:
  HASH_TABLE<SIG_TYPE,DATA_TYPE>(UINT num_elements);
#else
  MEM_POOL *_pool;
public:
  ...
  void Enter(SIG_TYPE signature,DATA_TYPE data) {
    ...
    HASH_ELEMENTP element = 
      new THIS_HASH_ELEMENT(signature,data);
#else
    HASH_ELEMENTP element = 
...
#ifdef USE_MALLOC
 public:
  USER_HASH_TABLE ( UINT32 num_elements );
#else
  MEM_POOL *_pool;
 public:
...
#ifdef USE_MALLOC
    HASH_ELEMENTP element = 
      new THIS_HASH_ELEMENT(key,data);
#else
    HASH_ELEMENTP element = 
...
template <class SIG_TYPE, class DATA_TYPE>
#ifdef USE_MALLOC
HASH_TABLE<SIG_TYPE,DATA_TYPE> :: HASH_TABLE (UINT num_elements )
#else
HASH_TABLE<SIG_TYPE,DATA_TYPE> :: HASH_TABLE (
UINT num_elements, MEM_POOL *pool )
#endif
{
  typedef HASH_ELEMENT<SIG_TYPE,DATA_TYPE> *HASH_ELEMENTP;
  ...
  _num_entries = 0;
#ifdef USE_MALLOC
  _data = new HASH_ELEMENTP[num_elements];
#else
  _pool = pool;
...
)
{
  ...
  _num_entries = 0;
#ifdef USE_MALLOC
  _data = new pHASH_ELEMENT[num_elements];
#else
  _pool = pool;
...
TEMPLATE_USER_HASH_TABLE
void
CLASS_USER_HASH_TABLE :: Remove (KEY_TYPE key ) 
{
  typedef HASH_ELEMENT<KEY_TYPE,DATA_TYPE> *HASH_ELEMENTP;
  UINT32 location = _hash ( key ) % _num_elements;
  HASH_ELEMENTP hash_element = _data[location];
  ...
  if ( _equal(hash_element->_signature, key) ) {
    _data[location] = hash_element->_next;
#ifdef USE_MALLOC
    delete(hash_element);
#else
    CXX_DELETE(hash_element,_pool);
