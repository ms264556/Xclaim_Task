...
/* Some systems want to override the above option, and our config
   system defines with the double underscores.  So fix that up.
*/

#ifdef __MALLOC_USE_SBRK__
# define MALLOC_USE_SBRK
#endif
...
/* The size of a malloc allocation is stored in a size_t word
   MALLOC_ALIGNMENT bytes prior to the start address of the allocation:
     ...
     +--------+---------+-------+-------------------+-------+
     | SIZE   |(unused) | GUARD | allocation  ...   | GUARD |
     +--------+---------+-------+-------------------+-------+
     ^ BASE                     ^ ADDR
     ^ ADDR - MALLOC_ALIGN
*/
...
/* The amount of extra space used by the malloc header and guard zone.  */
#define MALLOC_HEADER_SIZE	(MALLOC_ALIGNMENT + GUARD_ZONE_SIZE)
