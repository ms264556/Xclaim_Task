...
#ifdef _KEEP_RCS_ID
static char *cxx_templatercs_id = cxx_template_INCLUDED"$Revision: 1.25 $";
#endif /* _KEEP_RCS_ID */
...
public:
  ...
  T&      Get(mUINT32 idx) const 
    { 
      assert(idx < _lastidx && idx >= 0);
      return (*this)[idx]; 
    } // to access the array by idx
  void    Set(mUINT32 idx, const T& val) 
    { 
      assert(idx < _lastidx && idx >= 0);
      (*this)[idx]=val; 
...
//for compatability with stl vectors:
  mUINT32 size(void)  const              { return _lastidx + 1; }
  bool    empty(void) const              { return _lastidx == -1; }

  /* if we reserve space, then we know we will be putting size
     elements into the array. Go ahead and mark them as present.
     (by setting _lastidx). If we don't, then we may end up
     triggering the assertions in operator[] when we access
     them the first time to initialize them. */
  void    reserve(mUINT32 size)          
  { 
    Force_Alloc_array(size); 
    _lastidx = size - 1;
  }
  void    reserve_and_init(mUINT32 size)
  { 
    Force_Alloc_array(size); 
    Bzero_array();
    _lastidx = size - 1;
  }
  void    push_back(const T& val)        { AddElement(val); }
...
public:
  ...
  T       Pop(void) {
    ...
    Is_True(idx >= 0, ("STACK::pop(): Stack Empty"));
    t = _stack[idx];
    _stack.Decidx();
    return t;
  }
...
{
#ifdef USE_MALLOC
  _array = (T*)realloc(_array, sizeof(T) * new_size);
  if ( _array == NULL ) {
    fprintf(stderr, "Out of memory\n");
    profile_cleanup_and_exit();
  }
