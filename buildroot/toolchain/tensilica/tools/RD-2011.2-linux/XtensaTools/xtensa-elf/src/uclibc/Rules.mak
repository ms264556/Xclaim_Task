...
ifeq (SunOS,${findstring SunOS,${shell uname}})
INSTALL= $(XTENSA_SRC_DIR)/Target-libs/newlib/install-sh
else
INSTALL= install
endif
...
CONFIG_HAS_FLOATS=$(shell cat $(BUILDDIR).hasfloats)
...
ifeq ($(strip $(TARGET_ARCH)),xtensa)
        CPU_CFLAGS-$(CONFIG_GENERIC_XTENSA)+=-mlongcalls -I$(PREFIX)/xtensa-elf/arch/include \
		-I$(shell xt-run --show-config=xttools)/xtensa-elf/include
	# This should really be set by OS, rather than target, but because
	# debugging info doesn't make it onto the target in deeply embedded
	# systems, it's not useful to strip the object files on the way out.
	STRIPTOOL:= true
endif
...
# soft float encodings.
    CPU_CFLAGS += -msoft-float -msoft-double
ifeq ($(strip $(TARGET_ARCH)),arm)
...
CFLAGS=$(XWARNINGS) $(OPTIMIZATION) $(XARCH_CFLAGS) $(CPU_CFLAGS) \
	-fno-builtin -nostdinc -D_LIBC -I$(BUILDDIR)include -I.
