...
/* returns a bit mask of 'size' 1's */
#define _U4EXT_MASK(size) (_UINT32)(_U4SHL(1,size)-1)
#define _U8EXT_MASK(size) (_UINT64)(_U8SHL(1,size)-1LL)

/* returns a bit mask of 'size' 1's starting at 'offset' */
#define _U4CMP_MASK(offset, size) _U4SHL(_U4EXT_MASK(size),offset)
#define _U8CMP_MASK(offset, size) _U8SHL(_U8EXT_MASK(size),offset)

/* bit field extraction */
#define _I4EXTRACT_BITS(v, offset, size) \
        (_INT32)(((_U4LSHR(v,offset)) & _U4EXT_MASK(size)))
#define _U4EXTRACT_BITS(v, offset, size) \
        (_UINT32)(((_U4LSHR(v,offset)) & _U4EXT_MASK(size)))
#define _I8EXTRACT_BITS(v, offset, size) \
        (_INT64)(((_U8LSHR(v,offset)) & _U8EXT_MASK(size)))
#define _U8EXTRACT_BITS(v, offset, size) \
        (_UINT64)(((_U8LSHR(v,offset)) & _U8EXT_MASK(size)))

/* the value of v2 is deposited into the range of bits in v1 as
   specified by offset and size; the rest of the bits are taken from v1 */
#define _I4COMPOSE_BITS(v1, v2, offset, size) \
        (_INT32)(((v1) & ~_U4CMP_MASK(offset,size)) | \
                 (_U4SHL(v2,offset) & _U4CMP_MASK(offset,size)))
#define _U4COMPOSE_BITS(v1, v2, offset, size) \
        (_UINT32)(((v1) & ~_U4CMP_MASK(offset,size)) | \
                  (_U4SHL(v2,offset) & _U4CMP_MASK(offset,size)))
#define _I8COMPOSE_BITS(v1, v2, offset, size) \
        (_INT64)(((v1) & ~_U8CMP_MASK(offset,size)) | \
                 (_U8SHL(v2,offset) & _U8CMP_MASK(offset,size)))
#define _U8COMPOSE_BITS(v1, v2, offset, size) \
        (_UINT64)(((v1) & ~_U8CMP_MASK(offset,size)) | \
                 (_U8SHL(v2,offset) & _U8CMP_MASK(offset,size)))
