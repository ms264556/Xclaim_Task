...
/*
// Define WIN32 for NT 4.0
*/
#define _WIN32_WINNT 0x0400

/*
// This file provides driver routines for the XTMP System Simulation API.
// Instead of using SystemC (see mp_driver.cxx), we use NT fibers.
*/
/*
...
// If you use a pre-emptive thread package other than this one,
// you will need to re-implement the function cpuThreadCycle to match
// your thread package.
*/
typedef LPVOID FIBERP; /* pointer to fiber */
typedef struct _Thread Thread;
struct _Thread {
  FIBERP *fiber;			/* FIBER handle */
...
static void cpuThreadCycle(LPVOID);
static void threadStub(LPVOID);
static Thread *threadBaseNew( const char *,
...
static void *cpuThreadNew( const char *,
			   ...
			   void (*syncCPU)(XTMP_threadInfo *,XTMP_timeDelta),XTMP_threadInfo *);
...
internString( const char *s )
{
  char *interned;
  if( s ) {
    interned = (char *)malloc(strlen(s)+1);
    strcpy(interned,s);
    return interned;
  }
  return NULL;
...
eventNew(void)
{
  ...
  evt = (event_t)malloc(sizeof(struct event));
  evt->threads = NULL;
  evt->fired = false;
...
waitOnEvent_internal(XTMP_event event, Thread* thread)
{
  ...
  evt = (event_t)event;
  el = (evt_list_t)malloc(sizeof(struct evt_list));
  el->thread = thread;
  el->next = evt->threads;
  evt->threads = el;
}
...
void thread_heap_convert_from_wheel(void) {
  ...
  if (s_thread_heap == NULL) {
    s_thread_heap = (struct ThreadHeap *)malloc(sizeof(struct ThreadHeap));
    if (s_thread_heap == NULL) {
      fprintf( stderr, "qt_driver.c: allocation failure\n");
      exit(1);
    }
...
#if defined(DRIVER_TRACE)
  fprintf( stderr, "FIBER push( 0x%08x \"%s\", %d ) @ " PCT_LLD "\n",
...
*/
static Thread *
threadBaseNew( const char *name, LPFIBER_START_ROUTINE func,
			   unsigned stackSize )
{
  Thread *t = (Thread *)malloc(sizeof(Thread));
  assert( t!= 0 );

  t->fiber = CreateFiber(stackSize, func, t);
  t->next = NULL;
  t->link = allThreads;
#if USING_THREAD_HEAP
  t->heap_link = NULL;
...
threadNew( const char *name,
	   ...
	   XTMP_threadInfo *ti)
{
  Thread *t = threadBaseNew(name,(LPFIBER_START_ROUTINE)threadStub, 
		                    DEFAULT_STACK_SIZE);
  t->callBack.f = f;
  t->threadInfo = ti;
  t->cpuThread = false;
...
cpuThreadNew( const char *name,
	      ...
	      XTMP_threadInfo *ti)
{
  Thread *t = threadBaseNew(name, (LPFIBER_START_ROUTINE)cpuThreadCycle,
		                    DEFAULT_STACK_SIZE);
...
find_next_event_cycle()
{
  ...
  if (s_thread_heap) {
    if (s_thread_heap->top == NULL)
      return 0;
    return (u32)(s_thread_heap->top->heap_link_time - currentTime);
  }
#endif
  {
    int i, idx;
...
driverStop(void)
{
  ...
  if( numberOfThreads == 0 || currentThread == &mainThread
      || currentThread == NULL ) {
    fprintf( stderr, "fiber_driver.c: can't call XTMP_stop "
  ...
  if( numberOfThreads==0 ) {
	...
	SwitchToFiber( mainThread.fiber );
    abort();
  } else {
    currentThread = 0;
    next = advanceTime();
...
{
  int status = 0;
  mainThread.fiber = ConvertThreadToFiber(0);
