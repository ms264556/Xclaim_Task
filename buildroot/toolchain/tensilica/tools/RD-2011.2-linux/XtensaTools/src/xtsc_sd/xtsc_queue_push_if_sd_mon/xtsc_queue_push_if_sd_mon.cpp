...
#include <xtsc_sd/xtsc_queue_push_if_sd_mon.h>
...
class xtsc_queue_push_if_sd_monFactory : public MxFactory {
public:
  xtsc_queue_push_if_sd_monFactory() : MxFactory ("xtsc_queue_push_if_sd_mon") {}
  sc_mx_m_base *createInstance(sc_mx_m_base *c, const string &id) {
    ...
    return new xtsc_queue_push_if_sd_mon(c, id.c_str());
  }
};
...
extern "C" XTSC_SD_EXPORT void MxInit(void) {
  new xtsc_queue_push_if_sd_monFactory();
}
...
xtsc_sd::xtsc_queue_push_if_sd_mon::~xtsc_queue_push_if_sd_mon() { }
