...
// Configure, create, and connect xtsc_memory `$key`
--set_memory_parm=byte_width=`$byte_width`
--set_memory_parm=start_byte_address=` hx(8,$m->addr)`
--set_memory_parm=memory_byte_size=` hx(8,$m->sizem1+1)`
--set_memory_parm=read_only=`$m->ref_mems->[0]{mem}->is('writable') ? "false" : "true"`
--create_memory=`$key`
;   if ($num_memories == 1) {
;     if ($coherent) {
--connect_cohctrl_memory=cohctrl,`$key`
;     } else {
--connect_arbiter_memory=arbiter,`$key`
;     }
;   } else {
--connect_router_memory=router,`$memory_index`,`$key`
...
;   }
--core_program=$(`$core->name`_PROGRAM)
