...
void memory_size_check(u32 line_number) {
  u32 sysramsize = 0x00400000;
  u32 sysromsize = 0x00020000;
...
int sc_main(int argc, char *argv[]) {
  ...
  try {
    ...
    pif2sb_bridge_parms         bridge_parms            (4, 0x100);   // 0x100 = larger number = lower priority
    ...
    pif2sb_bridge_parms         traffic_bridge_parms    (4, 0x0);     // 0x0 = smaller number = higher priority
    pif2sb_bridge               traffic_bridge          ("traffic_bridge", traffic_bridge_parms);

    sc_clock                    clk                     ("clk");
    simple_bus                  bus                     ("bus");
    simple_bus_arbiter          arbiter                 ("arbiter");
    simple_bus_slow_mem         sysrom                  ("sysrom", 0x40000000, 0x4001FFFF, 2);  // 2 wait states on ROM
    simple_bus_fast_mem         sysram                  ("sysram", 0x60000000, 0x603FFFFF);
