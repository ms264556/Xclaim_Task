...
int sc_main(int argc, char *argv[]) {
  ...
  try {
    ...
    dma_parms.set("byte_width", 4);
    dma_parms.set("clear_notify_value", false);
    dma_parms.set("reg_base_address", 0xC0001000);
    ...
    core0_router->connect(*core0, "pif");

    // Connect core0_router to pif_arbiter
    pif_arbiter->connect(*core0_router, 0, 1);

    // Connect core0_router to port 0 of dma
    dma->connect(*core0_router, 1, 0);

    // Connect dma to dma_router
    dma_router->connect(*dma);

    // Connect dma_router to pif_arbiter
    pif_arbiter->connect(*dma_router, 0, 0);

    // Connect dma_router to core0
    dma_router->connect(*core0, "inbound_pif", 1);

    // Connect core0_router to mmio_arbiter
    mmio_arbiter->connect(*core0_router, 2, 0);

    // Connect dma_router to mmio_arbiter
    mmio_arbiter->connect(*dma_router, 2, 1);

    // Connect mmio_arbiter to mmio
    mmio->connect(*mmio_arbiter);

    // Connect mmio to core0
    mmio->connect(*core0, "BInterrupt00", "BInterrupt00");

    // Connect pif_arbiter to port 0 of pifmem
    pifmem->connect(*pif_arbiter, 0);

    // Connect core0 to dram0
    dram0->connect(*core0, "dram0ls0");
