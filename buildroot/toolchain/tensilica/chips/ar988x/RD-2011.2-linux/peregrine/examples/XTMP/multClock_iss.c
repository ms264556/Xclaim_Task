/*
 * Customer ID=8327; Build=0x3b95c; Copyright (c) 2004-2009 by Tensilica Inc.  ALL RIGHTS RESERVED.
 * 
...
XTMP_main(int argc, char **argv)
{
  ...
  shared_mem = XTMP_pifMemoryNew("shmem", pifWidth, bigEndian, 0x10000);
  ...
  XTMP_addMapEntry(router1, shared_mem, XTMP_BYPASS_PADDR, 0);
  XTMP_addMapEntry(router2, shared_mem, XTMP_BYPASS_PADDR, 0);
  ...
  XTMP_setClockFactor(core1, 4);
  if (XTMP_HAS_SYS_RAM) {
    XTMP_setReadDelay(ram1, 3);
    XTMP_setBlockReadDelays(ram1, 3, 3);
  }
  if (XTMP_HAS_SYS_ROM) {
    XTMP_setReadDelay(rom1, 3);
    XTMP_setBlockReadDelays(rom1, 3, 3);
  }
  XTMP_setReadDelay(shared_mem, 3);
