...
static bool
isByteEnabled(u16 byteEnables, XTMP_address addr, u32 byteWidth, bool bigEndian)
{
...
memPeekByteEn(void *deviceData, u32 *dst, XTMP_address addr, u16 byteEnables)
{
 ...
 */
static XTMP_deviceStatus
memPokeByteEn(void *deviceData, XTMP_address addr, const u32 *src, u16 byteEnables)
{
  LocalMemoryData *localMemory = (LocalMemoryData *) deviceData;
  u8 *buf = getByteAddress(localMemory, addr);
  u32 i;
...
memPost(void *deviceData, XTMP_deviceXfer *xfer)
{
  ...
  u16 byteEnables = XTMP_xferGetByteEnables(xfer);
