...
int sc_main(int argc, char *argv[]) {
  ...
  try {
    ...
    u32 bit_width = core0.get_tie_bit_width("status");
    template_wire_write_parms null_wire_parms(bit_width);
    null_wire_parms.extract_parms(argc, argv, "null_wire");
    template_wire_write null_wire("null_wire", null_wire_parms);

    // Connect the wire
    core0.get_export_state("status")(null_wire);
