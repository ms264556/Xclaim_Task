...
int sc_main(int argc, char *argv[]) {
  ...
  try {
    ...
    u32 bit_width = core0.get_tie_bit_width("INQ1");
    template_queue_pop_parms null_queue_parms(bit_width);
    null_queue_parms.extract_parms(argc, argv, "null_queue");
    template_queue_pop null_queue("null_queue", null_queue_parms);

    // Connect the queue
    core0.get_input_queue("INQ1")(null_queue.m_consumer);
    ...
    // Load programs
    core0.load_program("target/consumer.out");
