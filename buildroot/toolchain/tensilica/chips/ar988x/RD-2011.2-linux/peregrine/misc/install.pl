...
# Customer ID=8327; Build=0x3b95c; Copyright (c) 2004-2008 Tensilica Inc.  ALL RIGHTS RESERVED.
...
#  7.  move MSVC CRT assembly for pre-XP Windows? "yes" or "no"
...
if ( ! defined $ARGV[6] ) {
  die "INTERNAL ERROR: Argument missing; expecting move_vc_crt flag."
}
$xttools = $ARGV[0];
$config = $ARGV[1];
$interactive = $ARGV[2];
...
# For pre-XP versions of Windows, the MSVC CRT assembly that is installed
# in a subdirectory of lib/iss will not be recognized.  Move the contents
# up one level into the lib/iss directory so the DLLs will be found.  This
# must be done here, based on the version check in install.c, because it
# will break XP in at least some situations (contrary to Microsoft's own
# documentation, which implies that it ought to work).
if ("$move_vc_crt" eq "yes" && -d "$xttools/lib/iss/Microsoft.VC80.CRT") {
  while (glob "$xttools/lib/iss/Microsoft.VC80.CRT/*") {
    (my $newname = $_) =~ s/Microsoft.VC80.CRT//;
    rename $_, $newname or die "\nError: cannot move $_: $!\n";
  }
  rmdir "$xttools/lib/iss/Microsoft.VC80.CRT";
}
